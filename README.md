...
This project provides a collection of Scala-based utilities and shell scripts designed to automate common DevOps tasks. These tools help with managing Docker registries, transferring Docker images, initializing remote hosts, and running scripts on multiple servers.

## Docker Registry Cleaner (`DockerRegistryCleaner.scala`)

This utility helps keep your private Docker registry tidy by removing older image tags. For each repository, it retains only the 10 most recently pushed tags and deletes the rest.

**Configuration:**

The cleaner is configured through the `application.conf` file (typically located in `src/main/resources/`). You'll need to define the properties for your private Docker registry under a path like `repository.private.office`. This includes the registry URL, username, and password.
...
repository {
  private {
    office {
      ...
      # Other settings like realm, service, etc. if needed by RepositoryAPI
    }
  }
}
```
...
3. Pushes the retagged image to the target registry.
...
The `InitHost` utility is used to initialize remote hosts by adding a specified public SSH key to their `~/.ssh/authorized_keys` file. This allows for passwordless SSH access to the configured servers.
...
A simple example script that prints a greeting "Hello ${SELF_ENV}".
*   `SELF_ENV`: This environment variable can be set before running the script to customize the message.
...
./scripts/hello.sh
# or
SELF_ENV="World" ./scripts/hello.sh
```
...
*   `-n <数字>`: Total number of requests (default: 100).
*   `-s <秒>`: Sleep time in seconds between requests (default: 1).
*   `-t <秒>`: Timeout in seconds for each request (default: 5).
...
This project uses the HOCON (Human-Optimized Config Object Notation) format for configuration. The main configuration file is typically `src/main/resources/application.conf`.

**Key Configuration Points:**

*   **Docker Registry Details**: URLs, credentials for `DockerRegistryCleaner` and `DockerTransfer`. (e.g., `repository.private.office`, `repository.docker.host`)
*   **SSH Connection Info**: Hostnames, ports, usernames, keys/passwords for `InitHost` and `RunRemoteScript`. (e.g., `office.servers`, `office.devops.public`, and configurations for `ConfigShellRun`)
*   **Script-Specific Settings**: Any other parameters required by the individual utilities.

**Best Practices:**

*   It's advisable to create a `application.conf.example` file with placeholders for sensitive information or site-specific details. Users can then copy this to `application.conf` and fill in their actual values.
*   Ensure that `application.conf` is included in your `.gitignore` file if it contains sensitive credentials or private information that should not be committed to version control. The current `.gitignore` should be checked for this.
*   Refer to the specific documentation for each utility above for details on the configuration keys it uses.
...
*   **Compile:** Compile all Scala sources.
    ```bash
    sbt compile
    ```

*   **Run Tests:** Execute any unit tests in the project.
    ```bash
    sbt test
    ```

*   **Create a Package (JAR):** If needed, you can package the project (though typically these utilities are run directly via `sbt runMain`).
    ```bash
    sbt package
    ```
    This will create a JAR file in the `target/scala-<scala-version>/` directory.

## How to Run

**Scala Utilities:**

The Scala-based utilities (like `DockerRegistryCleaner`, `DockerTransfer`, `InitHost`, `RunRemoteScript`) are typically run using the `sbt runMain` command, followed by the object name and any necessary arguments.

**General Syntax:**

```bash
sbt "runMain <ObjectName> [arguments...]"
```

Refer to the specific documentation for each utility above for its exact run command and arguments.

**Shell Scripts:**

The shell scripts in the `scripts/` directory can be executed directly from your terminal, provided they have execute permissions (`chmod +x script_name.sh`).
